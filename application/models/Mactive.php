...
class Mactive extends CI_Model{
    ...
    public function __construct(){
        parent::__construct();
        log_message('info', 'Mactive Class start.');
    }
    
    public function select($type = 'active', $num = 10, $off = 0, $itemId = NULL){
        switch($type){
            case 'list':
            case 'index':
            ...
            case 'single':
            default:
                $this->db->where(array('code' => $num, 'active' => 1, 'time >= ' => strtotime('-2 hours')));
